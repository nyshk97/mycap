...
- サムネイルのキー（Esc / ⌘C / ⌘S / ⌘O / ⌘E / ⌘P）はホバー中と、出した直後の「待ち受け」中だけ効く。待ち受けは枠が光り、クリック・前面アプリの切り替え（撮影・Restore・編集の直後に元のアプリへ戻る分は除く）・5 秒・編集／履歴／録画を開く・マウスを乗せる、で解ける。マウスの移動では解けない。キーを持つサムネイルは常に 1 枚
