...
- 音（`Sources/App/Sounds.swift`。macOS 内蔵の音をパスで読む）: 撮ってサムネイルを出すとき Screen Capture.aif、録画の開始 begin_record.caf、停止 end_record.caf。OCR は鳴らさない。録画のシステム音は mycap 自身を除いているので開始音は動画に入らない
